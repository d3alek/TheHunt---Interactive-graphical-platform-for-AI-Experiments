...
public class D3Maths {
	...
	public static float[] quadBezierCurveVertices(float[] a,
		...
		float[] vertices = new float[D3GLES20.COORDS_PER_VERTEX * (verticesNum + 1)];
		float t = 0;
		for (int i = 0; i < verticesNum; i++, t += dt) {
			vertices[i * D3GLES20.COORDS_PER_VERTEX] = scale * (
					(1 - t) * (1 - t) * (1 - t) * a[0] 
					+ 3 * (1 - t) * (1 - t) * t * b[0]
					+ 3 * (1 - t) * t * t * c[0]
					+ t * t * t * d[0]);
			
			vertices[i * D3GLES20.COORDS_PER_VERTEX + 1] = scale * (
					(1 - t) * (1 - t) * (1 - t) * a[1] 
					+ 3 * (1 - t) * (1 - t) * t * b[1]
					+ 3 * (1 - t) * t * t * c[1]
					+ t * t * t * d[1]);
	
			vertices[i * D3GLES20.COORDS_PER_VERTEX + 2] = 0;
		}
		vertices[verticesNum * D3GLES20.COORDS_PER_VERTEX] = d[0] * scale;
		vertices[verticesNum * D3GLES20.COORDS_PER_VERTEX + 1] = d[1] * scale;
		vertices[verticesNum * D3GLES20.COORDS_PER_VERTEX + 2] = d[2] * scale;
		return vertices;
	}

	public static float[] circleVerticesData(float r, int verticesNum) {
		float[] vertices = new float[verticesNum*D3GLES20.COORDS_PER_VERTEX];
		float step = 2*3.14f/verticesNum;
		float angleRad = 0;
		for (int i = 0; i < verticesNum; ++i) {
			vertices[i*D3GLES20.COORDS_PER_VERTEX] = r * FloatMath.sin(angleRad);
			vertices[i*D3GLES20.COORDS_PER_VERTEX + 1] = r * FloatMath.cos(angleRad);
			vertices[i*D3GLES20.COORDS_PER_VERTEX + 2] = 0;
			angleRad += step;
		}
		return vertices;
